...
export default function AdminPage() {
  ...
  async function toggleDepositPaid(booking) {
    ...
    setAllBookings((prev) => prev.map((b) =>
      b.id === booking.id ? { ...b, deposit_paid: newPaid, deposit_paid_at: newPaid ? new Date().toISOString() : null } : b
    ))
  }
  ...
  async function saveMemberEdit() {
    ...
    if (editForm.name !== selectedMember.name) payload.name = editForm.name
    if (editForm.email !== selectedMember.email) payload.email = editForm.email
    if ((editForm.phone || '') !== (selectedMember.phone || '')) payload.phone = editForm.phone
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
              <div className="text-xs text-emerald-700">Betalda</div>
              <div className="text-lg font-bold text-emerald-700">
                {allBookings.filter((b) => b.deposit_paid && b.status === 'confirmed').length}
              </div>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <div className="text-xs text-amber-700">Obetalda</div>
              <div className="text-lg font-bold text-amber-700">
                {unpaidDeposits.length}
              </div>
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
              <div className="text-xs text-slate-500">Utestående</div>
